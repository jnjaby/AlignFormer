...
class RAFT(nn.Module):
    def __init__(self,
        ...
        if load_path:
            param_dict = torch.load(load_path, map_location=lambda storage, loc: storage)
...
if __name__ == '__main__':
    h, w = 256, 256
    # model = RAFT().cuda()
    model = FlowGenerator(load_path='../../experiments/pretrained_models/RAFT/raft-things.pth').cuda()
    model.eval()
    print(model)
