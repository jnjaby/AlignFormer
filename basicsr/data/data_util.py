...
def paired_paths_PSF_from_meta_info_file(folders, keys, meta_info_file,
    ...
    with open(meta_info_file, 'r') as fin:
        pair_names = [line[:-1].split(' ') for line in fin]
...
def multiple_paths_from_folder(folders, keys, filename_tmpl):
    """Generate multiple paths from folders.
    ...
    num_paths = {}
    for folder, key in zip(folders, keys):
        num_paths[key] = len(list(scandir(folder)))

    assert len(set(num_paths.values())), 'Numbers of files at each folder are not the same.'

    paths = []
    for gt_name in sorted(list(scandir(folders[0]))):
        d = {}
        for folder, key in zip(folders, keys):
            d[f'{key}_path'] = osp.join(folder, gt_name)
        paths.append(d)
