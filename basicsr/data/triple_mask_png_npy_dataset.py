...
class TripleMaskNpyPngImageDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        if self.file_client is None:
            ...
            self.npy_client = FileClient('npy')
        ...
        lq_path = self.paths[index]['lq_path'].replace('.png', '.npy')
        img_lq = self.npy_client.get(lq_path, 'lq')[..., [2, 1, 0]]
        ...
        if self.opt['phase'] == 'train':
            ...
            # random crop
            img_gt, img_lq, img_mask = random_crop([img_gt, img_lq,
                                                    img_mask], gt_size)

            # flip, rotation
            img_gt, img_lq, img_mask = augment([img_gt, img_lq,
                                                img_mask], self.opt['use_flip'], self.opt['use_rot'])
