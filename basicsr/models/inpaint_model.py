...
class InpaintModel(SRModel):
    """Base Inpaint model for image inpainting model."""
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.output = self.net_g(self.lq, self.light_mask)

        loss_weight = self.occlu_mask if hasattr(self, 'mask') else None

        l_total = 0
        loss_dict = OrderedDict()
        # pixel loss
        if self.cri_pix:
            l_pix = self.cri_pix(self.output, self.gt, weight=loss_weight)
            l_total += l_pix
    ...
    def test(self):
        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            with torch.no_grad():
                self.output = self.net_g_ema(self.lq, self.light_mask)
        else:
            self.net_g.eval()
            with torch.no_grad():
                self.output = self.net_g(self.lq, self.light_mask)
